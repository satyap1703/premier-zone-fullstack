...
const TeamData = () => {
  ...
  useEffect(() => {
    ...
    if (teamValue) {
      axios.get(`http://localhost:8081/player/v1?team=${encodeURIComponent(teamValue)}`)
        .then(response => {
      ...
      axios.get(`http://localhost:8081/player/v1?nation=${encodeURIComponent(nationValue)}`)
      .then(response => {
        setPlayerData(response.data);
        setLoading(false);
      })
      ...
      axios.get(`http://localhost:8081/player/v1?position=${encodeURIComponent(positionValue)}`)
      .then(response => {
        setPlayerData(response.data);
        setLoading(false);
      })
      ...
      axios.get(`http://localhost:8081/player/v1?name=${encodeURIComponent(nameValue)}`)
      .then(response => {
        setPlayerData(response.data);
        setLoading(false);
      })
